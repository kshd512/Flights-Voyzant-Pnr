...
import com.mmt.flights.common.enums.ErrorsEnum;
...
import com.mmt.flights.i5.odc.service.ODCCommonFlowSubscriber;
import com.mmt.flights.i5.odc.service.ODCUtil;
import com.mmt.flights.i5.odc.service.RequestType;
...
import com.mmt.flights.pnr.PnrServices;
import com.mmt.flights.pnr.config.TechConfig;
...
import static com.mmt.flights.common.constants.EndpointConstants.*;
...
public class ODCController {
    ...
    public DeferredResult<ResponseEntity<SimpleSearchResponseV2>> odcSearchV1(@RequestBody DateChangeSearchRequest request) {
        ...
        DeferredResult<ResponseEntity<SimpleSearchResponseV2>> deferredResult = new DeferredResult<>(techConfig.getPnrServReplyTimeOut());
        ...
        try {
            ...
            Observable<SimpleSearchResponseV2> observableResponse = pnrServices.odcSearchV1(request);
            observableResponse.subscribe(new ODCCommonFlowSubscriber(deferredResult, request, startTime, techConfig.getPnrServReplyTimeOut(), operation));
        } catch (Exception e) {
            PSErrorEnum errorCode = ErrorsEnum.INVALID_REQUEST;
    ...
    public DeferredResult<ResponseEntity<DateChangePrePaymentResponse>> prePayment(@RequestBody DateChangePrePaymentRequest request) {
        ...
        DeferredResult<ResponseEntity<DateChangePrePaymentResponse>> deferredResult = new DeferredResult<>(techConfig.getServiceReplyTimeOut());
        long startTime = System.currentTimeMillis();
        ...
        try {
            ...
            observableResponse.subscribe(new ODCCommonFlowSubscriber(deferredResult, request, startTime, techConfig.getServiceReplyTimeOut(), operation));
        } catch (Exception e) {
            PSErrorEnum errorCode = ErrorsEnum.INVALID_REQUEST;
            if (e instanceof PSErrorException) {
                errorCode = ((PSErrorException) e).getPsErrorEnum();
    ...
    public DeferredResult<ResponseEntity<DateChangeCommitResponse>> commit(@RequestBody DateChangeCommitRequest request) {
        ...
        DeferredResult<ResponseEntity<DateChangeCommitResponse>> deferredResult = new DeferredResult<>(techConfig.getServiceReplyTimeOut());
        long startTime = System.currentTimeMillis();
