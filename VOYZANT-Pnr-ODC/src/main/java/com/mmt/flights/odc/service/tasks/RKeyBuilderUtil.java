...
import com.mmt.flights.dotrez.pnr.pnrresponse.Leg;
import com.mmt.flights.dotrez.pnr.pnrresponse.ServiceChargeType;
...
public class RKeyBuilderUtil {
    ...
    private static RecommendationKeyFragment.Builder getBuilder(Journey journey,String cmsId, boolean rtJourney) {
        ...
        journey.getSegments().forEach(seg -> {
            ...
            String[] dateTime = DateTimeUtil.formatDate(seg.getDesignator().getArrival(), "ddMMyy hh:mm").split(" ");
            recommKeySegment.setArrDate(dateTime[0]);
            recommKeySegment.setArrTime(dateTime[1]);

            recommKeySegment.setDepCityCode(seg.getDesignator().getOrigin());
            String[] depDateTime = DateTimeUtil.formatDate(seg.getDesignator().getDeparture(), "ddMMyy hh:mm").split(" ");
            recommKeySegment.setDepDate(depDateTime[0]);
            recommKeySegment.setDepTime(depDateTime[1]);
    ...
    private static String getFareKey(Map<PaxType, SimpleFare> paxWiseFare, JourneyFare fare,JourneyFare rtJourneyFare, PaxCount paxCount) {
        ...
        builder.adultNumber((int) paxCount.getAdult()).childNumber((int) paxCount.getChild()).infantNumber((int) paxCount.getInfant());
    ...
    private static FareKeySegment getFareKeySegment(JourneyFare fare) {
        ...
        if (fare.getFares() != null && fare.getFares().size() > 0) {
            fareKeySegment.fareBasis(fare.getFares().get(0).getFareBasisCode());
            fareKeySegment.fareClass(fare.getFares().get(0).getClassOfService());
            fareKeySegment.productClass(fare.getFares().get(0).getProductClass());
            fareKeySegment.pax("A");
                        ...
                        fare1.getPassengerFares().forEach(passengerFare -> {
                            if (AdapterUtil.getSupplyPaxType(passengerFare.getPassengerType()) == SupplyPaxType.ADULT) {
                                passengerFare.getServiceCharges().forEach(c -> {
                                    if (c.getType() != null && c.getType() == ServiceChargeType.Tax && c.getCode() != null) {
                                        switch (c.getCode().toLowerCase()) {
                                            case "udf":
                                                fareKeySegment.udf(c.getAmount() + "");
                                                break;
                                            case "psf":
                                                fareKeySegment.psf(c.getAmount() + "");
                                                break;
                                            case "yq":
                                                fareKeySegment.yq(c.getAmount() + "");
                                                break;
                                            case "yr":
                                                fareKeySegment.yr(c.getAmount() + "");
                                                break;
                                        }
                                    }
                                });
    ...
    private static PaxFareInformation getPaxFareInformation(Map<PaxType, SimpleFare> paxWiseFare) {
        ...
        return new PaxFareInformation(new BigDecimal(adultTotal),
                new BigDecimal(childTotal), new BigDecimal(infantTotal), new BigDecimal(adultBase), new BigDecimal(childBase), new BigDecimal(infantBase));
    }
}
